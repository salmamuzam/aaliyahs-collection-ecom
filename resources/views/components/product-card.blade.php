...
<div wire:key="{{ $product->id }}" class="bg-white shadow-sm border border-gray-300 rounded-md p-3">
    <a href="/shop/{{ $product->id }}" class="block" wire:navigate>
        <div class="aspect-[3/4] overflow-hidden rounded-md">
            <img src="{{ url('storage', $product->images[0] ?? 'uploads/placeholder.png') }}"
                alt="{{ $product->name }}"
        ...
        <div wire:click.prevent="toggleFavorite({{ $product->id }})"
            ...
            @if ($isFavorite)
                <div class="relative flex items-center justify-center">
                    ...
                    {{-- Filled Heart --}}
                    <svg xmlns="http://www.w3.org/2000/svg" width="16px" class="fill-brand-burgundy animate-heart-filled relative z-10"
                        viewBox="0 0 24 24">
                        <path
            ...
            @else
                {{-- Outline Heart --}}
                <svg xmlns="http://www.w3.org/2000/svg" width="16px" class="fill-brand-burgundy inline-block"
                    viewBox="0 0 64 64">
                    <path

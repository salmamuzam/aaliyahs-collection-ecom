        ...
        <div class="relative max-w-6xl p-8 mx-auto bg-white rounded-lg shadow-lg">
            ...
            <form wire:submit="saveProduct" class="grid gap-5 mt-8">
                <div>
                    ...
                    <input {{ $isView ?  'disabled': 'enabled'}} wire:model="name" type='text' placeholder='Rose glow Abaya'
                ...
                <div class="col-span-full">
                    ...
                    <textarea  {{ $isView ?  'disabled': 'enabled'}} wire:model="description"
                        placeholder='Softly radiant and feminine — Rosé Glow features delicate hand-beaded detailing on the sleeves on overcoat with matching inner and shawl, crafted on a fluid peach-toned fabric that catches light effortlessly. A timeless choice for refined occasions.'
                        rows="6"
                ...
                <div class="relative w-full">
                    ...
                    <select  {{ $isView ?  'disabled': 'enabled'}} wire:model="category_id" id="categories"
                        class="w-full rounded-lg py-2.5 px-4 border border-slate-300 focus:border-[#822659] text-sm outline-none">
                ...
                <div>
                    ...
                    <input  {{ $isView ?  'disabled': 'enabled'}} wire:model="price" type='number' placeholder='Rs 26,500.00'
                        class="w-full rounded-lg py-2.5 px-4 border border-slate-300 focus:border-[#822659] text-sm outline-none" />
                    {{-- Capture Error Messages --}}
                    @error('price')
                        <p class="mt-2 text-sm font-medium text-rose-600">{{ $message }}</p>

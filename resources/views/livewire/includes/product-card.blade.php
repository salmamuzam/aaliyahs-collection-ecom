<div class="md:hidden p-6 bg-white shadow-sm border border-gray-300 rounded-md relative">
    <div class="flex items-center max-sm:flex-col gap-4 max-sm:gap-6">
        <div class="w-52 h-52 rounded-lg overflow-hidden shrink-0">
    <img src='{{ asset('storage/' . $product->image) }}' class="w-full h-auto object-cover" />
</div>

        <div class="sm:border-l sm:pl-4 sm:border-gray-300 w-full rounded-lg">
<div class="text-center">
  <h2 class="text-pink-600 text-sm font-medium bg-pink-50 px-3 py-1.5 inline-block tracking-wide rounded-lg">{{ $product->category->name }}</h2>
            ...
            <h3 class=" mt-4 text-left font-semibold text-slate-900">{{ $product->name }}</h3>


            <ul class=" text-sm text-slate-500 font-medium space-y-2">

                    <li class="text-rose-600 pt-4">Rs. {{ $product->price }}</li>
